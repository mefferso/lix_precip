...
def process_urma() -> dict[str, Any]:
    ...
    def build_urma_extreme(window: dict[str, datetime], mode: str) -> np.ndarray:
        ...
        stack_f = (np.array(grids) - 273.15) * 1.8 + 32.0
        return np.nanmax(stack_f, axis=0) if mode == "max" else np.nanmin(stack_f, axis=0)
...
def build_dynamic_title(dataset_key: str, config: dict[str, Any], grid_data: dict[str, Any] | None = None) -> str:
    ...
    if dataset_key in ("air_temp_daily_min", "air_temp_daily_max"):
        ...
        return f"Low Temperatures This Morning - {start_local.strftime('%A, %B')} {start_local.day}, {start_local.year}" if start_local.date() == today_local else f"Low Temperatures - {start_local.strftime('%A, %B')} {start_local.day}, {start_local.year}"
