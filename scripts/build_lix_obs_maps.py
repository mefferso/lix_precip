...
def get_current_obs_time_info() -> dict[str, datetime] | None:
    ...
    try:
        ...
        valid_times = pd.to_datetime(df["valid_time"], utc=True, errors="coerce").dropna().sort_values()
    except Exception as e:
...
def get_latest_daily_temp_windows(now_utc: datetime) -> dict[str, dict[str, datetime]]:
    ...
    def full_local_day(day):
        ...
        return {"start_local": start_local, "end_local": end_local, "start_utc": start_local.astimezone(timezone.utc), "end_utc": end_local.astimezone(timezone.utc)}
    def partial_today():
...
def build_dynamic_title(dataset_key: str, config: dict[str, Any], grid_data: dict[str, Any] | None = None) -> str:
    ...
    if dataset_key == "precip_24h":
        ...
        return f"24 Hour Rainfall ({format_month_day_year(start_local)} @ {format_ampm(start_local)} - {format_month_day_year(end_local)} @ {format_ampm(end_local)}) - Observed"
