...
# URMA can lag current obs by several hours. First try to match obs time closely;
# then fall back to latest available URMA instead of drawing ugly station triangles.
...
DATASETS: dict[str, dict[str, Any]] = {
    ...
    "air_temp_latest": {
        ...
        "desc": "URMA Gridded Background Matched to Current Station Observation Time",
    },
    "air_temp_daily_min": {
...
def get_current_obs_time_info() -> dict[str, datetime] | None:
    ...
    ns_values = valid_times.astype("int64").to_numpy(dtype=np.int64)
    median_ns = int(np.nanmedian(ns_values))
    median_utc = pd.to_datetime(median_ns, unit="ns", utc=True).to_pydatetime()
    target_hour_utc = median_utc.replace(minute=0, second=0, microsecond=0)
    min_utc = valid_times.min().to_pydatetime()
    max_utc = valid_times.max().to_pydatetime()

    print(
        "Current temp obs times: "
        f"median={median_utc.strftime('%Y%m%d %H:%MZ')}, "
        f"range={min_utc.strftime('%Y%m%d %H:%MZ')} to {max_utc.strftime('%Y%m%d %H:%MZ')}, "
        f"target URMA hour={target_hour_utc.strftime('%Y%m%d %HZ')}"
    )
...
def unique_hours(hours: list[datetime]) -> list[datetime]:
    ...
    out: list[datetime] = []
    for dt in hours:
        hr = dt.astimezone(timezone.utc).replace(minute=0, second=0, microsecond=0)
        if hr in seen:
            continue
        seen.add(hr)
        out.append(hr)
...
def get_latest_daily_temp_windows(now_utc: datetime) -> dict[str, dict[str, datetime]]:
    ...
    high_window = full_local_day(yesterday) if now_local.hour < 16 else full_local_day(today)
    low_window = partial_today() if now_local.hour >= 9 else full_local_day(yesterday)
    return {"air_temp_daily_max": high_window, "air_temp_daily_min": low_window}


def build_urma_extreme(window: dict[str, datetime], mode: str) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    stack_f = np.array(grids)
    extreme = np.nanmax(stack_f, axis=0) if mode == "max" else np.nanmin(stack_f, axis=0)
    return extreme, lon_ref, lat_ref


def process_urma() -> dict[str, Any]:
    now_utc = datetime.now(timezone.utc).replace(minute=0, second=0, microsecond=0)
    obs_info = get_current_obs_time_info()
    target_dt = obs_info["target_hour_utc"] if obs_info else now_utc

    result: dict[str, Any] = {}
    if obs_info:
        result["air_temp_latest_obs_median_utc"] = obs_info["median_utc"]
    ...
    try:
        ...
        result["air_temp_daily_max"] = max_f
        if lon_for_xy is None or lat_for_xy is None:
            lon_for_xy, lat_for_xy = lon_max, lat_max
    except Exception as e:
        print(f"URMA daily max failed: {e}")

    try:
        min_f, lon_min, lat_min = build_urma_extreme(windows["air_temp_daily_min"], "min")
        result["air_temp_daily_min"] = min_f
        if lon_for_xy is None or lat_for_xy is None:
            lon_for_xy, lat_for_xy = lon_min, lat_min
    except Exception as e:
        print(f"URMA daily min failed: {e}")

    if lon_for_xy is not None and lat_for_xy is not None:
        transformer = Transformer.from_crs("EPSG:4326", "EPSG:3857", always_xy=True)
        x_3857, y_3857 = transformer.transform(lon_for_xy, lat_for_xy)
        result["x"] = x_3857
        result["y"] = y_3857
...
def build_dataset_rows(dataset_key: str, config: dict[str, Any], manual: dict[str, list[str]]) -> pd.DataFrame:
    ...
    df["auto_outlier"] = modified_zscore_flags(values, threshold=4.5) | buddy_check_flags(
        lats, lons, values, config["neighbor_radius_km"], config["neighbor_min"], config["buddy_threshold"]
    )
...
def load_geography(plot_bbox: tuple[float, float, float, float] = PLOT_BBOX) -> GeoContext:
    ...
    return GeoContext(
        ...
        index_domain.to_crs(TARGET_CRS),
        cities.to_crs(TARGET_CRS),
    )
...
def triangle_edge_lengths_km(x: np.ndarray, y: np.ndarray, triangles: np.ndarray) -> np.ndarray:
    ...
    return np.maximum.reduce([a, b, c]) / 1000.0
...
def thin_precip_label_points(df_g: gpd.GeoDataFrame, value_col: str, max_labels: int = 110, min_distance_km: float = 8.0) -> gpd.GeoDataFrame:
    ...
    for _, row in work.iterrows():
        ...
        if any(((x - sx) ** 2 + (y - sy) ** 2) ** 0.5 < required_dist_m for sx, sy in selected_xy):
            continue
...
def thin_label_points_by_distance(df_g: gpd.GeoDataFrame, value_col: str, max_labels: int = 150, min_distance_km: float = 12.0) -> gpd.GeoDataFrame:
    ...
    if not selected_rows:
        return work.head(0).drop(columns=["_sort_x", "_sort_y"], errors="ignore")
    return gpd.GeoDataFrame(selected_rows, crs=df_g.crs).drop(columns=["_sort_x", "_sort_y"], errors="ignore")
...
def format_ampm_tz(dt: datetime) -> str:
    local = dt.astimezone(LOCAL_TZ)
    hour = local.strftime("%I").lstrip("0")
    if local.minute == 0:
        return f"{hour} {local.strftime('%p %Z')}"
    return f"{hour}:{local.minute:02d} {local.strftime('%p %Z')}"
...
def get_observed_24h_window_local() -> tuple[datetime, datetime]:
    ...
    end_local = now_local.replace(hour=OBS_WINDOW_END_LOCAL_HOUR, minute=0, second=0, microsecond=0)
...
def build_dynamic_title(dataset_key: str, config: dict[str, Any], grid_data: dict[str, Any] | None = None) -> str:
    ...
    if dataset_key in ("air_temp_daily_min", "air_temp_daily_max"):
        ...
        if start_local.date() == today_local:
            return f"Low Temperatures This Morning - {start_local.strftime('%A, %B')} {start_local.day}, {start_local.year}"
        return f"Low Temperatures - {start_local.strftime('%A, %B')} {start_local.day}, {start_local.year}"
...
def plot_dataset(dataset_key: str, config: dict[str, Any], geo: GeoContext, manual: dict[str, list[str]], grid_data: dict[str, Any], region_key: str = "full", region_config: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if is_regional:
        ...
        ax_head.text(0.5, 0.1, dynamic_title, ha="center", va="center", fontsize=18 if dataset_key in ("precip_24h", "air_temp_latest") else 22, fontweight="bold", style="italic")
    ...
    if dataset_key in grid_data and "x" in grid_data and "y" in grid_data:
        ax.contourf(
            grid_data.get(f"{dataset_key}_x", grid_data.get("x")),
            grid_data.get(f"{dataset_key}_y", grid_data.get("y")),
    ...
    elif dataset_key != "air_temp_latest":
...
def main() -> None:
    ...
    manifest: dict[str, Any] = {
        "regions": {key: {"label": value["label"], "bbox": value["bbox"]} for key, value in REGIONS.items()},
        "maps": {},
        "current_temperature_timing": {
            key: value.isoformat() if isinstance(value, datetime) else value
            for key, value in grid_data.items()
            if key.startswith("air_temp_latest_") and key.endswith("_utc")
        },
    }
